...
IF NOT EXIST "%NUGETEXE%" (
    ...
    FOR %%i in (%NUGETPACKAGESTOINSTALL%) DO (
        ECHO.
        ECHO ####################################################################################################
        ECHO # Begin installing latest NuGet package for: %%i
        ECHO ####################################################################################################
        ECHO.
        "%NUGETEXE%" install %%i -Source https://msdyneng.pkgs.visualstudio.com/_packaging/AXApplication-Rel/nuget/v3/index.json -OutputDirectory "%NUGETPACKAGEINSTALLDIR%" -NonInteractive -DirectDownload
        ECHO.
        ECHO ####################################################################################################
