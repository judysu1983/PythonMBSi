...
IF NOT EXIST "%NUGETEXE%" (
    ...
    IF NOT EXIST "%NUGETPACKAGEINSTALLDIR%" ( md "%NUGETPACKAGEINSTALLDIR%" )
        ...
        ECHO ####################################################################################################
        ECHO.
        "%NUGETEXE%" install Microsoft.Dynamics.AX.CostAccountingProd.Translations -Version 8.1.6 -Source https://msdyneng.pkgs.visualstudio.com/_packaging/AXApplication-Rel/nuget/v3/index.json -OutputDirectory "%NUGETPACKAGEINSTALLDIR%" -NonInteractive -DirectDownload
        ECHO.
        ECHO ####################################################################################################
        ECHO # Finished installing latest NuGet package for: %%i
        ECHO ####################################################################################################
